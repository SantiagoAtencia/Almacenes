...
WEBSERVER_PATH = "src/web-server.py" #webserver.py path, relative to the test file:
...
class WebServerClient:
    ...
    def increment(self, objeto:str, quantity:int)->dict:
        ...
        msg = {"accion": "annadir", "nombre": objeto, "cantidad": quantity}
        response = requests.post(f"{self.base_url}accion", json=msg)
        logging.info(response.json())
        assert response.status_code == 200
        return response.json()
    ...
    def decrement(self, objeto:str, quantity:int)->dict:
        ...
        msg = {"accion": "sacar", "nombre": objeto, "cantidad": quantity}
        response = requests.post(f"{self.base_url}accion", json=msg)
        logging.info(response.json())
        assert response.status_code == 200
        return response.json()
    ...
    def get_inventory(self)->dict:
        ...
        response = requests.get(f"{self.base_url}inventario/ver")
        logging.info(response.json())
        assert response.status_code == 200
        return response.json()
    ...
    def get_item_quantity(self, objeto:str)->int:
        ...
        response = requests.get(f"{self.base_url}inventario/{objeto}")
        logging.info(response.json())
